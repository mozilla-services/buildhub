...
snippets =
    [ { title = "Is this an official buildid?"
      ...
      , snippets =
            { curl = """curl -s -I "https://kinto-ota.dev.mozaws.net/v1/buckets/build-hub/collections/archives/records?build.id=20110110192031&_limit=1" | grep "Total-Records: 1" """
...
"""
    ...
    , { title = "What is the revision of a build id?"
      , description = ""
      , snippets =
            { curl = """curl -s "https://kinto-ota.dev.mozaws.net/v1/buckets/build-hub/collections/archives/records?build.id=20110110192031&_limit=1" | grep -Po '"revision":"[^"]+' | sed 's/"revision":"//'"""
            , js = """import KintoClient from "kinto-http";
...
revision = records[0]["source"]["revision"]"""
    ...
    , { title = "What are the locales of a specific version?"
      , description = ""
      , snippets =
            { curl = """curl -s "https://kinto-ota.dev.mozaws.net/v1/buckets/build-hub/collections/archives/records?target.version=53.0b9&_fields=target.locale" | grep -Po '"locale":"[^"]+' | sed 's/"locale":"//' | sort -u"""
            , js = """import KintoClient from "kinto-http";
